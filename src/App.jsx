...
function Caixa({ cash, setCash, loading }) {
  ...
  // Saldo acumulado por dia (sobre TODOS os lançamentos)
  const allDays = [...new Set(cash.map(c => c.date))].sort();
  const saldoAcumulado = {};
  let runningBal = 0;
  allDays.forEach(d => {
    cash.filter(c => c.date === d).forEach(c => { runningBal += c.type === "entrada" ? c.value : -c.value; });
    saldoAcumulado[d] = runningBal;
  });
